	...
	while(get_lines(buf,888,in)){
		...
		if(key==0){
			...
			if(area>=0&&area<=30){
				...
				for(test_n=strlen(buf)-1;;--test_n){
					if(isspace(buf[test_n])==0)
						break;
				}
				//test_n=str_buf.rfind("\n",str_buf.size()-1);
				test_n=test_n+1;
				if(back1==-1&&back2==-1){
					strcat(subject,str_to_char(str_buf.substr(9,test_n-9)));
				}else{
					back2=back2+2;
			...
			if(str_buf.find(boundary,0)<=100&&str_buf.find(boundary,0)>=0){
				...
				if(need_decode==1){
					...
					strcpy(return_content,str_to_char((base64_decode(str_buf))));
				}
				continue;
			}	
		...
		if(key==0){
			...
			if(area>=0&&area<=100){
				...
				if(back1>=0&&back2>=0){
					...
					for(back2=strlen(buf)-1;back2>=0;--back2){
						if(isspace(buf[back2])==0){
							buf[back2+1]='\0';
							break;
						}
					}
					for(back2=area+9;back2<=strlen(buf)-1;++back2)
						*attachment_subject++=buf[back2];
					*attachment_subject='\0';
				}
				++count;
				continue;
			}	
...
int get_attachment_from_num(char* file_number,char* attachment_way,int model){
	...
	char attachment_file_name[1024]; //array_attachment_way;
	//strcpy(attachment_file_name,array_attachment_way);

	int attachment_way_size=strlen(attachment_way);
	for(i=0;i!=attachment_way_size;++i)
		array_attachment_way[i]=*attachment_way++;
	array_attachment_way[i] = '\0';
	cout << "The subject is: " << attachment_subject << endl;
	strcat(array_attachment_way,attachment_subject);
	ofstream out(array_attachment_way,ios::out|ios::binary);
	char decode_buf[9];
